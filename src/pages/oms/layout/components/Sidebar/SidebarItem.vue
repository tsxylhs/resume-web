    ...
    template(v-for='item in routes', v-if='!item.hidden&&item.children')
      router-link(v-if='hasOneShowingChildren(item.children) && !item.children[0].children&&!item.alwaysShow', :to="item.path+'/'+item.children[0].path", :key='item.children[0].name')
        el-menu-item(:index="item.path+'/'+item.children[0].path", :class="{'submenu-title-noDropdown':!isNest}")
          svg-icon.fill-white(v-if='item.children[0].meta&&item.children[0].meta.icon', :icon-class='item.children[0].meta.icon')
          span(v-if='item.children[0].meta&&item.children[0].meta.title', slot='title') {{item.children[0].meta.title}}
      el-submenu(v-else='', :index='item.name||item.path', :key='item.name')
        template(slot='title')
          svg-icon.fill-white(v-if='item.meta&&item.meta.icon', :icon-class='item.meta.icon')
